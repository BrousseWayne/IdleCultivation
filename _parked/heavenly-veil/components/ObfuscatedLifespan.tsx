...
const VOID_CHARS = ["█", "▆", "▄", "▂"];
const ZOOM_EVENT_THRESHOLD = 1.3;
const ZOOM_EVENT_RESET_THRESHOLD = 1.2;
...
export function ObfuscatedLifespan({ value, className = "" }: ObfuscatedLifespanProps) {
  ...
  useEffect(() => {
    if (zoomLevel > ZOOM_EVENT_THRESHOLD && !hasEmittedEvent.current) {
      hasEmittedEvent.current = true;
      EventBus.emit({
        type: "player:peered_at_fate",
        zoomLevel,
      });
    } else if (zoomLevel < ZOOM_EVENT_RESET_THRESHOLD && hasEmittedEvent.current) {
      hasEmittedEvent.current = false;
    }
  }, [zoomLevel]);

  useEffect(() => {
    if (normalizedZoom < 0.2) return;
  ...
  const displayValue = useMemo(() => {
    ...
    return str.split('').map((char, i) => {
      const phase = Math.sin(i * 2.5 + animationFrame * 0.2) * 0.18;
  ...
  }, [value, corruption, animationFrame]);
  ...
  return (
    <span className={`relative inline-block ${className}`} style={{ minWidth: '1.5em' }}>
      <span
      ...
      <span
        ...
        style={{
          background: corruption > 0.65
            ? `linear-gradient(${(animationFrame * 8) % 360}deg, rgba(255, 40, 80, 0.95), rgba(40, 255, 255, 0.95) 33%, rgba(255, 255, 40, 0.95) 66%, rgba(255, 40, 255, 0.95))`
          ...
          transform: corruption > 0.65
            ? `scale(${1 + corruption * 0.45}) rotate(${Math.sin(animationFrame * 0.1) * corruption * 12}deg) skew(${Math.cos(animationFrame * 0.08) * corruption * 8}deg)`
            : undefined,
