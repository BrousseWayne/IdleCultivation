...
- WIRED: `SaveManager` (`src/game/services/SaveManager.ts`) — localStorage `cultivation-save`, `SAVE_VERSION = 2` (v1 saves migrate on load), 30s autosave. What gets saved/restored is declared per store in the **persistence manifest** (`services/persistence.ts`), which `SaveManager` and `reincarnate()` (via `resetRunState()`) both iterate; unlock flags merge OVER data defaults so content added after a save stays visible. `notificationStore` is intentionally absent from the manifest.
